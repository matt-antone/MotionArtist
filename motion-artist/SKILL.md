...
| `--performer` | optional | `female`, `male` or `neutral` — how the motion reads, carried into `motion.json` and the manifest. The first two are the filmed body; `neutral` says the move reads the same either way and is an answer, not a blank. **It describes the trace, never the character to draw.** CAG has no such input and nothing downstream renders off it — it is there so a person or an agent picking motions out of the library can filter them. When the clip came from `clipper`, take this from the clip's `performer` rather than choosing it; a clip marked before the field existed has none, and then omit the flag rather than guessing. |
...
3. Render:
   ...
   `render` has no `--pingpong` of its own: the capture owns the answer and `render` reads it, so a
   capture cut without the flag is flipped by re-cutting it or editing `motion.json`, not at render
   time. It adds no cells and leaves
