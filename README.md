...
| `--stabilize` | centre the hips horizontally in every frame; use for a panning camera or a travelling performer. Body scale is always normalised per frame (torso length to the clip median), so camera zoom never changes the skeleton's size |
